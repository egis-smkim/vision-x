	...
	dbscan.getClusters = function() {
		...
		for (let i = 0; i < num_clusters; i++) {
			...
			if (distance === haversine_distance) {
				clusters_centers[i] = { location: { latitude: 0, longitude: 0} };
				
				var gids = [];
				for (let j = 0; j < clusters[i].length; j++) {
					clusters_centers[i].location.latitude += data[clusters[i][j]].lat;
					clusters_centers[i].location.longitude += data[clusters[i][j]].lon;
					gids.push(data[clusters[i][j]].gid);
				}

				clusters_centers[i].dimension = clusters[i].length;
				//clusters_centers[i].parts = clusters[i];
				clusters_centers[i].gids=gids;
				...
				clusters_centers[i].indx=timeStampe;
		...
		var clusterWithNoise = {
			cluster:clusters_centers,
			noise:noises
		}

		return clusterWithNoise;
	};

	dbscan.noises=function(d){
		if (Array.isArray(d)) {
			data = d;
		}
	}
