var D_MEMBER={
    ...
    LOGIN:{
        ...
        executeLogin:function() {
            ...
            fetch("data/en-US.json")
            .then(response => response.json())
            .then(data => {
                var Login = data.Login;
                if($("#memId").val() == "") {
                    D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.no_id.param1, "", "info", function() { $("#memId").focus(); });
                    return false;
                }

                if($("#memPwd").val() == "") {
                    D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.no_password.param1, "", "info", function() { $("#memPwd").focus(); });
                    return false;
                }
                            ...
                            if(result.MEM_INFO == "NONE") {
                                if(result.PWD == "INCORRECT"){
                                    if(result.login_count<5){
                                        ...
                                        //D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.pwd_unmatched.param1'), Login.popper.login.alert_message.login_validation.pwd_unmatched.param2') + result.login_count + Login.popper.login.alert_message.login_validation.pwd_unmatched.param3'), "error", function() { });
                                        D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.id_umatched.param1, "", "error", function() { });
                                        return false;
                                    }else{
                                        LOG_TRACKER.write("13","2","로그인 5회 실패로 인한 비밀번호 초기화:{ID:"+mem_id+"}");
                                        $("#errorPwdPopup").addClass("active");
                                ...
                                D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.unauthorized.param1, Login.popper.login.alert_message.login_validation.unauthorized.param2, "error", function() { });
                                return false;
                            }
                            else if(result.MEM_INFO.MEM_LEVEL == -1) {
                                LOG_TRACKER.write("11","1","이메일 미인증 사용자:{ID:"+mem_id+"}");
                                D_MEMBER.openAlert(Login.popper.login.alert_message.login_validation.unauthenticated.param1, Login.popper.login.alert_message.login_validation.unauthenticated.param2, "info", function() { });
                                return false;
                            }
                            else {
                                if((domain_host != false && (domain_host == "admin" || domain_host.indexOf("mgt") > -1)) && result.MEM_INFO.MEM_LEVEL != 10){
                                    ...
                                    D_MEMBER.openAlert(Login.popper.login. alert_message.login_validation.unauthorized_admin.param1, "", "error", function() { });
                                    return false;
                                }else{
    ...
    SIGNUP:{
        ...
        executeSignUp:function() {
            ...
            .then(data => {
                ...
                if($("#joinMemId").val() == "") {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_id, "", "info", function() { $("#joinMemId").focus(); });
                    return false;
                }else if($("#joinMemId").val().length<6){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_id_length, "", "info", function() { $("#joinMemId").focus(); });
                    return false;
                }else if(idRule == null){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_id_rule, "", "info", function() { $("#joinMemId").focus(); });
                    return false;
                }else if($("#joinMemId").val().indexOf("admin") > -1 ||
                    $("#joinMemId").val().indexOf("root") > -1 ||
                    $("#joinMemId").val().indexOf("manager") > -1){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_id_blacklist, "", "info", function() { $("#joinMemId").focus(); });
                    return false;
                }

                if($("#joinMemName").val() == "") {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_name, "", "info", function() { $("#joinMemName").focus(); });
                    return false;
                }

                var emailRule = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i;
                if($("#joinMemEmail").val() == "") {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_email, "", "info", function() { $("#joinMemEmail").focus(); });
                    return false;
                }else if(emailRule.test($("#joinMemEmail").val())==false){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_email_reg, "", "info", function() { $("#joinMemEmail").focus(); });
                    return false;
                }

                var pwdRule = /(?=.*[~`!@#$%\^&*()-+=]{1,50})(?=.*[0-9]{1,50})(?=.*[a-zA-Z]{1,50}).{9,50}$/;
                if($("#joinMemPwd").val() == "") {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_password, "", "info", function() { $("#joinMemPwd").focus(); });
                    return false;
                }else if(pwdRule.test($("#joinMemPwd").val())==false){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_password_reg, "", "info", function() { $("#joinMemPwd").focus(); });
                    return false;
                }else if(D_MEMBER.SIGNUP.checkPasswordContinued($("#joinMemPwd").val())){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_password_reg_consecutive, Login.popper.join.alert_message.join_validation.no_password_reg_consecutive2, "info", function() { return false; });
                    return false;
                }else if(($("#joinMemId").val() == $("#joinMemPwd").val()) || ($("#joinMemEmail").val() != "" && $("#joinMemEmail").val().split("@")[0] == $("#joinMemPwd").val())){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_password_reg_dup, Login.popper.join.alert_message.join_validation.no_password_reg_dup2, "info", function() { return false; });
                    return false;
                }else if(D_MEMBER.SIGNUP.checkPasswordId($("#joinMemPwd").val(),$("#joinMemId").val())){
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_password_reg_dup3, Login.popper.join.alert_message.join_validation.no_password_reg_dup4, "info", function() { return false; });
                    return false;
                }


                if($("#joinMemPwdCfm").val() == "") {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_passwordconf, "", "info", function() { $("#joinMemPwdCfm").focus(); });
                    return false;
                }

                if($("#joinMemPwdCfm").val() != $("#joinMemPwd").val()) {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_passwordconf_reg, "", "info", function() { $("#joinMemPwdCfm").focus(); });
                    return false;
                }

                if(!$("#memInfoAgr").is(":checked")) {
                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.no_consent2, "", "info", function() { $("#memInfoAgr").focus(); });
                    return false;
                }
                ...
                $.ajax({
                    ...
                    success : function(result) {
                        if (result.rs != 1) {
                                        ...
                                        $.ajax({
                                            ...
                                            success : function(result2) {
                                                if (result2.rs == "complete") {
                                                    D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.success, Login.popper.join.alert_message.join_validation.success2, "success", function() {
                                                        //location.reload();
                                                    });
                                                }
                                            }
                            ...
                            D_MEMBER.openAlert(Login.popper.join.alert_message.join_validation.alreadyExist, "", "error", function() { });
                            return false;
                        }
                    }
                });
    ...
    FINDINFO :{
        excuteFindPwd:function(){
            ...
            $("#memInfoForm").validate({
                ...
                messages:{
                    findMemId : {
                        required : Login.popper.findpwd.alert_message.no_id
                    },
                    findMemName : {
                        required : Login.popper.findpwd.alert_message.no_name
                    },
                    findMemEmail : {
                        required : Login.popper.findpwd.alert_message.no_email,
                        email : Login.popper.findpwd.alert_message.no_email_reg
                    }
                },
                submitHandler(form){
                    ...
                    $.ajax({
                        ...
                        success : function(result) {
                            if(result.rs=="complete"){
                                D_MEMBER.openAlert(Login.popper.findpwd.alert_message.success, Login.popper.findpwd.alert_message.success2, "success", function() {
                                    //location.reload();
                                });
                            }else if(result.rs=="fail"){
                                D_MEMBER.openAlert(Login.popper.findpwd.alert_message.no_match_id, "", "error", function() { });
                            }else if(result.rs=="error"){
                                D_MEMBER.openAlert(Login.popper.findpwd.alert_message.error, Login.popper.findpwd.alert_message.error2, "error", function() { });
                            }
                        }
                    });
                }
